...
namespace Lib
{
    ...
    {
        public static bool IsRightTrangle(int x1, int y1, int x2, int y2, int x3, int y3)
        {
            ...
            if (!IsTrangle(len1, len2, len3)) return false;

            return ((len1 * len1 + len2 * len2 == len3 * len3) |
                    (len2 * len2 + len3 * len3 == len1 * len1) |
                    (len3 * len3 + len1 * len1 == len2 * len2));
        }
        public static bool IsTrangle(int len1, int len2, int len3)
        {
